    ...
    if (it->second) {
      ...
      switch (it->second->type) {
        ...
        case TouchEvent::TYPE_MOVE:
          ...
          c = ofColor::blue;
          break;
        case TouchEvent::TYPE_EXIT:
          type = "EXIT";
          c = ofColor::red;
          break;
        case TouchEvent::TYPE_ENTER:
          type = "ENTER";
          c = ofColor::green;
          break;
        case TouchEvent::TYPE_CLICK:
          type = "CLICK";
          c = ofColor::magenta;
          break;
      }
...
void TouchManager::dispatchTouchMove(int id, const ofVec2f &p) {
  ...
  event->travelDistance +=
      fabs(event->prevPosition.x - event->position.x) + fabs(event->prevPosition.y - event->position.y); // manhattan distance to save some cpu
  ...
  if (node != event->receiver && event->lastSeenAbove == event->receiver) {
    // touch just went out of the origin receiver area,
    // send touchExit
    event->type = TouchEvent::TYPE_EXIT;
    event->receiver->touchExit(id, event);
  } else if (node == event->receiver && event->lastSeenAbove != event->receiver) {
...
ofVec2f TouchManager::getRandomTouchPosition() {
  return ofVec2f(ofRandom(touchRainArea.getLeft(), touchRainArea.getRight()), ofRandom(touchRainArea.getTop(), touchRainArea.getBottom()));
}
