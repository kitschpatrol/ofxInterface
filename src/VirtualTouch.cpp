...
VirtualTouch::VirtualTouch() : fingerTex(NULL), bInitialized(false), isTap(false) {
}
...
void VirtualTouch::draw() {
  ...
  fingerTex->draw(-fingerTex->getWidth() * fingerTexHotSpotAnchor.x, -fingerTex->getHeight() * fingerTexHotSpotAnchor.y);
  ofPopMatrix();
}
