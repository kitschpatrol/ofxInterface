...
public:
  ...
  }                      // please override with update code
  ...
  void setAllowOnlyOneTouch(bool set) {
    bNodeAllowOneTouch = set;
  }
  void setUpdateWhenHidden(bool set) {
    bNodeUpdateWhenHidden = set;
  }
  ...
  void onPositionChanged() {
    ofNotifyEvent(eventPositionChanged, *this);
  }
  void onScaleChanged() {
    ofNotifyEvent(eventScaleChanged, *this);
  }
  void onOrientationChanged() {
    ofNotifyEvent(eventOrientationChanged, *this);
  }
  ...
  float getPlane() const {
    return plane;
  }
  void setPlane(float _plane) {
    plane = _plane;
  }
  float getGlobalPlane() const {
    return (parent == NULL) ? plane : ((Node *)parent)->getGlobalPlane() + plane;
  }
  ...
  virtual void setSize(float w, float h) {
    size.set(w, h);
    ofNotifyEvent(eventNodeSizeChanged, *this, this);
  }
  ...
  float getLocalWidth() {
    return getSize().x * getScale().x;
  }
  float getLocalHeight() {
    return getSize().y * getScale().y;
  }
  float getGlobalHeight() {
    return getSize().y * getGlobalScale().y;
  }
  float getGlobalWidth() {
    return getSize().x * getGlobalScale().x;
  }

  ofVec2f getCenter() {
    return getPosition() + size * 0.5f;
  }
  ofVec2f getGlobalCenter() {
    return toGlobal(getPosition() + size * 0.5f);
  }
...
*/
  ...
  virtual bool acceptsNode(Node *node) {
    return false;
  }
...
private:
  template <typename ArgType> ofEvent<ArgType> *getEventForName(const string &eventName) {
